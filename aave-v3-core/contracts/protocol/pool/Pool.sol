...
contract Pool is VersionedInitializable, PoolStorage, IPool {
  ...
  /// @inheritdoc IPool
  /**
   * @dev     bridge시 다른 체인으로 전송하는 함수
   * @param   asset  보낼 토큰
   * @param   amount  보낼 양
   * @param   onBehalfOf  토큰을 받을 주소
   * @param   referralCode  이벤트로 발생시킬 레퍼럴코드(외부기능 추가용)
   */
  ...
  function getReservesList() external view virtual override returns (address[] memory) {
    ...
    for (uint256 i = 0; i < reservesListCount; i++) {
      // 리스트에서 0 주소로 설정된 곳이 있다면 다음 index에 해당하는 주소로 채우기
    ...
    assembly {
      // reserveList 배열의 크기를 비어있는 부분들을 제외하여 저장
      // 배열의 0 번째 인덱스에는 배열의 크기가 들어간다
